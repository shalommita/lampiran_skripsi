...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $_GET['search'];
    $searchCondition = "WHERE waktuCatat LIKE '%$search%' OR kategori LIKE '%$search%' OR skor LIKE '%$search%' OR catatanEval LIKE '%$search%'";
}

$sql = "SELECT
            CONCAT(waktuCatat, ' - ', kategori) AS waktuCatat,
            skor,
            kategori,
            catatanEval
        FROM (
                    ...
                    <table class="table" id="myformPemantau">
                        ...
                        <tbody style="text-align: center;">
                            ...
                            if ($result) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo '<tr>';
                                    // Convert the date format from Y-m-d H:i:s to d/m/Y - Kategori
                                    $formattedDate = date('d/m/Y', strtotime(substr($row['waktuCatat'], 0, 19)));
                                    $kategori = explode(" - ", $row['waktuCatat'])[1];
